...
// Binds a live MediaStream (from the real WebRTC call engine) to a <video>
// element. Renders nothing (returns null-equivalent empty fragment styling)
// when there's no stream yet, so the existing decorative gradient/avatar
...
const StreamVideo: React.FC<{
  ...
  mirrored?: boolean;
  className?: string;
}> = ({ stream, muted, mirrored, className }) => {
  ...
  return (
    <video
      ...
      playsInline
      muted={muted}
      className={`${className || ""} ${hasVideoTrack ? "opacity-100" : "opacity-0 pointer-events-none"} transition-opacity duration-300 ${mirrored ? "-scale-x-100" : ""}`}
    />
  );
};
...
export default function VideoCallScreen() {
  ...
  // NOTE: state transitions (outgoing -> connecting -> active) are now driven
  // by the real WebRTC call engine (see store/callStore.ts), not a demo timer.
  ...
  // NOTE: networkQuality is now driven by the real RTCPeerConnection state
  // (see store/callStore.ts wiring to callEngine's connectionStateChange).
        ...
        <div className="absolute inset-0 flex flex-col lg:flex-row lg:p-6 lg:pb-32 gap-4 lg:gap-6 bg-black">
          ...
          <div className="hidden lg:block relative flex-1 rounded-3xl overflow-hidden shadow-2xl border border-white/20">
            ...
            >
              ...
              <StreamVideo
                stream={store.isCameraOff ? null : store.localStream}
                muted
                mirrored={store.cameraFacing === "front"}
                className="absolute inset-0 w-full h-full object-cover z-[1]"
              />
              <div
