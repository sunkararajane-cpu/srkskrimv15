...
export function CommunityFeed({
  ...
  // notified" set in localStorage so re-renders/reloads don't re-fire it.
  useEffect(() => {
    if (!joined) return;
    const seenKey = `world_admin_notified_${world.id || "default"}`;
    let seen: string[] = [];
    try {
      seen = JSON.parse(localStorage.getItem(seenKey) || "[]");
    } catch {}

    const newAdminPosts = posts.filter(
      (p) =>
        (p.type === "announcement" || p.type === "event") &&
        p.author &&
        p.author !== "You" &&
        (p.authorLevel === "admin" || p.author === "SkrimAdmin") &&
        !seen.includes(p.id)
    );
    ...
    newAdminPosts.forEach((p) => {
      useSignalStore.getState().addSignal({
        ...
        text:
          p.type === "event"
            ? `posted a new event in ${world.name}: "${p.eventTitle}"`
            : `posted an announcement in ${world.name}: "${(p.content || "").slice(0, 80)}"`,
        time: "Just now",
        worldId: world.id,
      });
    });
    ...
    try {
      localStorage.setItem(seenKey, JSON.stringify([...seen, ...newAdminPosts.map((p) => p.id)]));
    } catch {}
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [posts, joined, world.id]);
                ...
                <div className="bg-[#151520] border border-white/5 p-3 rounded-xl flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="bg-[#1A1A24] text-white text-xs font-bold border border-white/10 rounded-xl px-3 py-2 outline-none focus:border-white/20 transition-all w-[110px]"
                    />
                    <button
                      type="button"
                      onClick={(e) => {
                        e.preventDefault();
                        if (scheduleDate) {
                          setScheduledFor(`${scheduleDate}T${scheduleTime || "00:00"}`);
                        }
