...
function resolveSparkMentions(rawHandles: string[], selfUsername?: string) {
  ...
  rawHandles.forEach((raw) => {
    ...
    if (!clean || clean === self || seen.has(clean)) return;
    seen.add(clean);

    const match = mockUsers.find((u) => u.username?.toLowerCase() === clean);
    resolved.push({
      user: match?.displayName || raw.replace('@', ''),
      handle: `@${clean}`,
...
export function SparkCreator({ isOpen, onClose, onPost, respondingToChallenge, respondingToChain }: SparkCreatorProps) {
  ...
  const handlePost = async () => {
    ...
    // Auto-moderation gate: scan attached media before this Spark is ever
    // posted. Flagged media blocks the post automatically — no confirmation
    // from the poster and no manual team review is required.
    if (mode !== 'text' && selectedMedia?.url && (selectedMedia.type === 'image' || selectedMedia.type === 'video')) {
      const result = await scanMedia(selectedMedia.url, selectedMedia.type as 'image' | 'video');
      if (result.flagged) {
        useModerationLogStore.getState().logAutoBlock({
          surface: 'spark',
          mediaKind: selectedMedia.type as 'image' | 'video',
          score: result.score,
          categories: result.categories,
          source: result.source,
          userHandle: currentUser?.username,
        });
        alert('🚫 This Spark was removed automatically for violating our adult-content guidelines.');
        return;
      }
    }
    ...
    // Notify anyone @mentioned in the text/caption or tagged on the photo.
    resolveSparkMentions(
