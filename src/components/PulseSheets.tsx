  ...
  const handleShareAsSpark = async () => {
    ...
    try {
      ...
      if (!stored.some(s => s.id === sparkId)) {
        ...
        const thumbnail = post.image || post.images?.[0] || post.image1 || null;
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-2xl border border-white/10 bg-white/5">
                    {(post.image || post.images?.[0]) && (
                      <img src={post.image || post.images?.[0] || post.image1 || null} alt="" className="w-12 h-12 rounded-lg object-cover shrink-0" />
                    )}
  ...
  const handleShareAsVibe = async () => {
    ...
    try {
      ...
      const newVibe = {
        ...
        thumbnail: post.image || post.thumbnail || post.image1 || '',
