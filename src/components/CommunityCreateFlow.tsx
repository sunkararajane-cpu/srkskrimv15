...
import { seedMockPendingRequests } from "../hooks/useWorldMembership";
...
export function CommunityCreateFlow({ onClose }: CreateFlowProps) {
  ...
  const handleLaunch = () => {
    ...
    // Give a private world's admin something real to approve/deny right away.
    if (formData.isPrivate) {
      seedMockPendingRequests(newCommunity.id);
    }
...
function Step1Name({ data, onChange, onNext, activeColors }: any) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col min-h-full">
...
function Step5Paid({ data, onChange, onNext, activeColors }: any) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col min-h-full">
      <h2 className="text-xl font-bold text-white flex items-center gap-2 mb-4"><span className="text-[#888899]">✦</span> Who can find & join this World?</h2>

      <div className="flex flex-col gap-3 mb-6">
         <div
           onClick={() => onChange({ isPrivate: false })}
           className={`p-4 rounded-xl border-2 transition-all cursor-pointer relative flex items-start gap-4 ${!data.isPrivate ? 'bg-white/[0.03]' : 'bg-transparent border-white/5 hover:border-white/10'}`}
           style={!data.isPrivate ? { borderColor: activeColors[0] } : {}}
         >
           {!data.isPrivate && <div className="absolute top-0 right-0 p-2"><Check className="w-4 h-4" style={{ color: activeColors[0] }} /></div>}
           <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-xl shrink-0">🌍</div>
           <div>
             <h4 className="text-[15px] font-bold text-white mb-1">PUBLIC WORLD</h4>
             <p className="text-[13px] text-[#888899]">Anyone can find it in Discover/search and join instantly.</p>
           </div>
         </div>
         ...
         >
           ...
           <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-xl shrink-0">🔒</div>
           <div>
             <h4 className="text-[15px] font-bold text-white mb-1">PRIVATE WORLD</h4>
             <p className="text-[13px] text-[#888899]">Hidden from Discover/search. People must request to join, and you approve or deny each request.</p>
           </div>
         </div>
      </div>
