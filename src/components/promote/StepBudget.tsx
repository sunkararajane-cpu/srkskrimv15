  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        {/* Exact start/end dates — an alternative to the quick day chips above */}
        <div className="flex gap-2 mt-3">
          <div className="flex-1">
            <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Start date</label>
            <input
              type="date"
              min={todayIso()}
              ...
              onChange={(e) => {
                ...
                const end = draft.endDate && draft.endDate > start ? draft.endDate : isoPlusDays(start, duration);
                onChange({ startDate: start, endDate: end, duration: diffDaysIso(start, end) });
              }}
              className="w-full bg-skrim-surface border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white outline-none focus:border-neon-purple/50"
            />
          </div>
          <div className="flex-1">
            <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">End date</label>
            <input
              type="date"
              min={draft.startDate || todayIso()}
              value={draft.endDate || isoPlusDays(draft.startDate || todayIso(), duration)}
              onChange={(e) => {
      ...
      <div className="bg-skrim-surface rounded-2xl border border-white/5 p-4">
        ...
        <div className="flex flex-col gap-2 text-[12px]">
          ...
          <Row label="Language" value={draft.targeting.languages.length > 0 ? draft.targeting.languages.join(', ') : 'All languages'} />
          <Row label="Device" value={draft.targeting.device === 'all' ? 'All devices' : draft.targeting.device === 'ios' ? 'iOS' : 'Android'} />
          <Row label="Frequency cap" value={`${draft.targeting.frequencyCap}x / person / day`} />
          <Row label="Goal" value={draft.goal.charAt(0).toUpperCase() + draft.goal.slice(1)} />
          {draft.destinationValue && <Row label="Destination" value={draft.destinationValue} />}
        </div>
      </div>
    </div>
  );
