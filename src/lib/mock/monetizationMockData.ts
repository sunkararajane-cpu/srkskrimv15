...
export interface AdTargeting {
  scope: "radius" | "city" | "state" | "country" | "worldwide";
  radiusKm: number;
  country: string;
  state: string | null;
  city: string | null;
  ...
  frequencyCap: number; // max times the same person sees this ad per day
}
...
// Languages available for targeting (Indian languages + English, matching Skrim's user base)
...
export type AdGoalId = "clicks" | "calls" | "whatsapp" | "followers" | "boost";

export interface AdGoalMeta {
  id: AdGoalId;
  label: string;
  ctaDefault: string;
  destinationLabel: string | null;
  destinationType: "url" | "phone" | "none";
  placeholder: string;
}

// Ad goals — flat-rate, no conversion tracking. Each goal just determines
// which destination field to collect and what the CTA button says/does.
export const AD_GOALS: AdGoalMeta[] = [
  { id: "clicks", label: "Get Website Clicks", ctaDefault: "Visit Website", destinationLabel: "Website / Link URL", destinationType: "url", placeholder: "https://yourwebsite.com" },
  { id: "calls", label: "Get Phone Calls", ctaDefault: "Call Now", destinationLabel: "Phone Number", destinationType: "phone", placeholder: "+91 98765 43210" },
  { id: "whatsapp", label: "Get WhatsApp Messages", ctaDefault: "Message on WhatsApp", destinationLabel: "WhatsApp Number", destinationType: "phone", placeholder: "+91 98765 43210" },
  { id: "followers", label: "Get More Followers", ctaDefault: "Follow", destinationLabel: null, destinationType: "none", placeholder: "" },
  { id: "boost", label: "Boost This Post", ctaDefault: "Learn More", destinationLabel: null, destinationType: "none", placeholder: "" },
];

export interface AdDraft {
  format: "video" | "post" | "story" | "carousel" | null;
  creativeId: string | null;
  creativeSource: "existing" | "upload";
  uploadedMedia: string[]; // base64 data URLs — single item unless format is carousel
  headline: string;
  ctaText: string;
  goal: AdGoalId;
...
export const AD_DRAFT_DEFAULTS: AdDraft = {
  ...
  destinationValue: "",
  targeting: {
    scope: "city", radiusKm: 10, country: "India", state: null, city: null,
    ageMin: 18, ageMax: 45, gender: "all", interests: [],
    languages: [], device: "all", frequencyCap: 3,
  },
  duration: 1,
  startDate: null,
  endDate: null,
