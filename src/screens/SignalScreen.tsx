...
import { Bell, MessageCircle, UserPlus, CheckCircle, Zap, Settings as SettingsIcon, X, Megaphone } from 'lucide-react';
            ...
            <div key={notif.id} className={`flex items-center gap-4 py-4 border-b border-white/5 ${!notif.isRead ? 'bg-neon-purple/5 -mx-4 px-4' : ''}`}>
              ...
              >
                 ...
                 <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-skrim-surface flex items-center justify-center border-2 border-black">
                   ...
                   {notif.type === 'world_join' && <span className="text-[10px]">🔒</span>}
                   {notif.type === 'game_invite' && <span className="text-[10px]">🎮</span>}
                   {notif.type === 'world_admin' && <Megaphone className="w-2.5 h-2.5 text-orange-400" />}
                   {notif.type === 'message' && <MessageCircle className="w-2.5 h-2.5 text-emerald-400" />}
                 </div>
              </div>
              <div className="flex-1" onClick={() => {
                 ...
                 if (notif.type === 'grind_reminder') {
                   ...
                   notif.vibeId &&
                   ['new_vibe', 'vibe_like', 'vibe_comment', 'vibe_reply', 'pulse', 'comment', 'mention'].includes(notif.type)
                 ) {
                   // `vibeId` is reused for Sparks too (reactions, replies, and
                   // Spark mentions all stamp the Spark's id into this field),
                   // so check there first and only fall back to the Vibes
                   // feed route when it isn't actually a Spark.
                   const mySparksStr = localStorage.getItem('skrimchat_sparks');
                   let spark: any = null;
                   if (mySparksStr) {
                     try {
                       const mySparks = JSON.parse(mySparksStr);
                       spark = mySparks.find((s: any) => s.id === notif.vibeId);
                     } catch (e) {}
                   }
                   if (spark) {
                   ...
                   navigate(`/profile/${notif.user.replace(/\s+/g, '_').toLowerCase()}`);
                 } else if (notif.type === 'world_join' && notif.worldId) {
                   navigate(`/world/${notif.worldId}`);
                 } else if (notif.type === 'game_invite' && notif.chatId) {
                   navigate(`/chat/${notif.chatId}`);
                 } else if (notif.type === 'world_admin' && notif.worldId) {
                   navigate(`/world/${notif.worldId}`);
