...
export default function CoinWalletScreen() {
  ...
  return (
    <div className="h-screen bg-[#080810] text-white flex flex-col overflow-hidden">
      ...
      {/* Balance card */}
      <div className="mx-4 mt-3 rounded-3xl p-4 bg-gradient-to-br from-[#B026FF]/20 via-[#7C3AED]/10 to-transparent border border-[#B026FF]/20 relative overflow-hidden shrink-0">
        ...
        <div className="flex items-end gap-2 mb-3">
          ...
          <span className="text-[#D4AF37] text-lg font-bold mb-1">coins</span>
        </div>
      ...
      {/* Tabs */}
      <div className="flex gap-2 px-4 mt-3 border-b border-white/5 pb-0 shrink-0">
        {(['history', 'earn', 'buy'] as const).map(tab => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 text-sm font-bold capitalize rounded-t-xl transition-all border-b-2 ${
              activeTab === tab
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3 pb-8">
          ...
          <div className="flex flex-col gap-1.5 h-full">
            {log.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center">
                <div className="w-14 h-14 rounded-full bg-white/5 flex items-center justify-center mb-3 text-2xl">🪙</div>
              ...
              <motion.div
                ...
                className="flex items-center gap-3 p-2.5 bg-white/5 rounded-2xl border border-white/5"
              >
        ...
        {activeTab === 'earn' && (
          <div className="flex flex-col gap-2">
            <p className="text-white/50 text-sm mb-1">Ways to earn Skrim Coins</p>
            {HOW_TO_EARN.map((item, i) => (
              <motion.div
                ...
                className="flex items-center gap-3 p-2.5 bg-white/5 rounded-2xl border border-white/5"
              >
                <div className="w-10 h-10 rounded-2xl bg-[#B026FF]/10 border border-[#B026FF]/20 flex items-center justify-center text-xl shrink-0">
                  {item.icon}
                </div>
            ...
            <button
              ...
              className="mt-1 w-full py-3 rounded-2xl bg-[#B026FF] text-white font-bold text-sm flex items-center justify-center gap-2 hover:bg-[#9a1fe0] transition active:scale-95"
            >
        ...
        {activeTab === 'buy' && (
          <div className="flex flex-col gap-2">
            <p className="text-white/50 text-sm mb-1">Purchase coins to use in Promote campaigns</p>
            {COIN_PACKS.map((pack, i) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="text-white font-bold text-sm">{formatCoins(pack.coins)} coins</p>
                      {pack.tag && (
                ...
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-xl font-bold text-sm transition active:scale-95 ${
                    pack.highlight
            ...
            <div className="mt-1 bg-white/5 rounded-2xl p-2.5 border border-white/5">
