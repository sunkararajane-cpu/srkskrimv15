...
import { useWorlds, useWorldMembership, getPendingRequests, approveJoinRequest, denyJoinRequest } from "../hooks/useWorldMembership";
...
export default function WorldDetailScreen() {
  ...
  const handleJoinClick = () => {
    if (joined || isJoining || pending) return;
      ...
      <div className="sticky top-[env(safe-area-inset-top,20px)+56px] z-40 bg-[#0A0A14]/90 backdrop-blur-xl border-b border-white/5 px-4 py-3 flex items-center gap-3 mt-4">
        <div className="flex-1 relative">
          ...
          <motion.button
            ...
            style={
              !joined && !world.paid && !pending
                ? {
                    background: `linear-gradient(to right, ${colors[0]}, ${colors[1]})`,
                  }
                : {}
            }
            disabled={isJoining || isLeaving || pending}
          >
...
function AboutTab({
  ...
  return (
    ...
    >
      ...
      {/* Pending Join Requests (private worlds, admin-only) */}
      {isAdmin && world.isPrivate && (
        <div>
          <div className="bg-[#111115] rounded-2xl border border-white/5 overflow-hidden shadow-sm">
            <div className="bg-white/5 px-4 py-3 border-b border-white/5 flex items-center gap-2">
              <span className="text-lg">🔒</span>
            ...
            <div className="p-4 flex flex-col gap-3">
              {pendingRequests.length === 0 ? (
                <p className="text-[13px] text-[#888899] text-center py-2">No pending requests right now.</p>
              ) : (
                pendingRequests.map((req) => (
                  <div key={req.id} className="flex items-center justify-between gap-3 bg-[#13131D] border border-white/[0.02] p-3 rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center text-[13px] font-bold text-white shrink-0">
                        {req.name.slice(0, 1)}
                      </div>
                      <div>
                        <p className="text-[13px] font-bold text-white">{req.name}</p>
                        <p className="text-[11px] text-[#888899]">Requested to join</p>
                      </div>
                    </div>
