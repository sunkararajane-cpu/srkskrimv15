...
function WorldCard({
  ...
  const navigate = useNavigate();
  const { joined, join, pending, requestJoin } = useWorldMembership(world.id);
  ...
  const handleJoinClick = (e: React.MouseEvent) => {
    ...
    if (world.isPrivate) {
      // Private worlds don't get instant-join fanfare — just send the
      // request and let the small ripple confirm it went through.
      setRipples([1]);
      requestJoin();
      setTimeout(() => setRipples([]), 700);
      return;
    }
        ...
        <div>
          <div className="flex items-center gap-2 mb-1 relative">
            ...
            <h3 className="text-[18px] font-bold text-white leading-tight flex items-center gap-2">
              {world.name}
              {world.isPrivate && (
                <span className="text-[10px] font-bold text-white/70 px-1.5 py-0.5 rounded-sm bg-white/10 ml-1 flex items-center gap-1">
                  🔒 Private
                </span>
              )}
              {world.paid && (
            ...
            {!joined ? (
              <div className="relative">
                ...
                <motion.button
                  ...
                  disabled={pending}
                  className={`backdrop-blur-lg text-[12px] font-bold px-4 py-1.5 rounded-full border shadow-[0_4px_12px_rgba(0,0,0,0.2)] transition-all shrink-0 relative z-10 flex items-center gap-1.5 ${
                    pending
