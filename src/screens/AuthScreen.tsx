...
export default function AuthScreen() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!dobCheck.valid) { setError(dobCheck.error || "Please enter a valid date of birth"); setLoading(false); return; }
    if (dobCheck.isMinor && !guardianConsent) {
      setError("Since you're under 18, please confirm parent/guardian consent to continue");
      setLoading(false);
      return;
    }
  ...
  const handleProfileSetupSubmit = (e: React.FormEvent) => {
    ...
    const newUser = { id: `user_${Date.now()}`, phone: fullPhone, fullName, username: formattedUsername, bio, avatar: finalAvatar, dateOfBirth, isMinor: dobCheck.isMinor };
          ...
          >
            <div className="flex-1 overflow-y-auto no-scrollbar pt-16 lg:flex-none lg:overflow-visible lg:pt-0">
              ...
              <form onSubmit={handleSignup} className="space-y-4">
                ...
                <div>
                  <label className="text-xs font-medium text-gray-400 ml-2">Date of Birth</label>
                  <input
                    type="date"
                    value={dateOfBirth}
                    ...
                    className="w-full bg-skrim-surface border border-gray-800 rounded-xl px-4 py-3 mt-1 focus:outline-none focus:border-neon-purple focus:ring-1 focus:ring-neon-purple/50 transition-all text-white [color-scheme:dark]"
                  />
                  {dateOfBirth && validateDOB(dateOfBirth).valid && validateDOB(dateOfBirth).isMinor && (
                    <label className="flex items-start gap-2 mt-2 ml-1 cursor-pointer select-none">
                      <input type="checkbox" checked={guardianConsent} onChange={e => setGuardianConsent(e.target.checked)} className="mt-0.5 w-4 h-4 accent-[#B026FF]" />
                      <span className="text-xs text-gray-400 leading-relaxed">
                        I confirm a parent or guardian is aware and consents to this account, as required for users under 18.
                      </span>
                    </label>
                  )}
                </div>
